...
/// 3. Creates ranking snapshots for the current date
/// 
/// # Arguments
...
pub async fn collect_top_coins(
    ...
    tracing::info!("Starting top {} coins collection job", limit);

    // Create CoinGecko connector
    let connector = CoinGeckoConnector::new();

    // Fetch top coins
    let coins = match connector.fetch_top_coins(limit).await {
        Ok(coins) => coins,
        Err(e) => {
            tracing::error!("Failed to fetch top coins from CoinGecko: {}", e);
            return Ok(CollectionResult {
                ...
                assets_updated: 0,
                rankings_created: 0,
                error: Some(format!("Failed to fetch coins: {}", e)),
            });
        }
    };
    ...
    for coin in coins {
        ...
        // Check if ranking already exists for this asset/date/source
        let existing_ranking = asset_rankings::Entity::find()
            .filter(asset_rankings::Column::AssetId.eq(asset_id))
            .filter(asset_rankings::Column::SnapshotDate.eq(snapshot_date))
            .filter(asset_rankings::Column::Source.eq(source))
            .one(db)
            .await?;

        if existing_ranking.is_some() {
            tracing::debug!(
                "Ranking already exists for asset {} on date {}, skipping",
                coin.symbol,
                snapshot_date
            );
            continue;
        ...
        new_ranking.insert(db).await?;
        rankings_created += 1;
        tracing::debug!(
            "Created ranking: {} (rank {}) on {}",
            coin.symbol,
            rank,
            snapshot_date
        );
    ...
    tracing::info!(
        "Top coins collection completed: {} collected, {} assets created, {} updated, {} rankings created",
        coins_collected,
        assets_created,
        assets_updated,
        rankings_created
