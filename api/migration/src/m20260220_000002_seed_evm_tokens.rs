...
/// Seeds the `evm_tokens` table with a curated list of well-known ERC-20 tokens across all
/// supported EVM chains.  These are the same tokens that were previously baked into the
/// `get_common_tokens()` function in `connectors/evm.rs`.
///
/// Operators can extend or modify this data at any time via the `/api/v1/evm-tokens` API,
/// or let the `POST /api/v1/evm-tokens/sync-from-contracts` endpoint auto-populate tokens
/// from the `asset_contracts` table (updated nightly by the `fetch_all_coins` job).
...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // Remove only the seed rows; manually-added rows are left untouched.
        for (_chain, _symbol, address) in SEED_TOKENS {
            db.execute(Statement::from_sql_and_values(
                DbBackend::Postgres,
                "DELETE FROM evm_tokens WHERE contract_address = $1",
